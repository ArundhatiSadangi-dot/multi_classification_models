...
LOCAL_DATA = Path("data/winequality-white.csv")
...
# ----------------------------
def load_wine_white():
    # If cached locally, use it; else read from UCI (semicolon-separated)
    if LOCAL_DATA.exists():
        df = pd.read_csv(LOCAL_DATA, sep=';')
    else:
        df = pd.read_csv(DATA_URL, sep=';')
    # Features/target
    X = df.drop(columns=["quality"])
    y = df["quality"].astype(int)  # multiclass labels (typically 3-9)
    return df, X, y
...
def build_models():
    models = {
        "Logistic Regression": Pipeline([
            ...
            ("clf", LogisticRegression(max_iter=1000, multi_class="ovr", n_jobs=-1, random_state=RANDOM_STATE))
        ]),
